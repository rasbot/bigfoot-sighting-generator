...
def split_tweet(observation, sighting_info=None, only_obs=False, numbered=True, max_length=235):
    o = ""
...
def tweet():
    ...
    with open("gpt2_sightings.json") as f:
        data = json.load(f)
    ...
    sighting_info = None
    observation = """OBSERVATION: {obs}""".format(obs=random_sighting)

    # delete sighting from JSON file
    del data[random_key]
    with open("gpt2_sightings.json", "w") as f:

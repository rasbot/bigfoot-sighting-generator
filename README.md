...
Currently the twitter bot is running on an EC2 instance, and will run a script once every 4 hours. The EC2 instance does not need to be running 24 hours a day, so setting up Lambda to schedule starting and stopping the instance can be used to limit usage time.

Another step would be to extend the project to a popular website for UFO sightings. This could be very entertaining to train a bot to tweet hybrid sightings trained on both sighting data sets. The results would be sightings of large, hairy creatures from beyond the stars. Also the EDA would be interesting to compare UFO sightings to bigfoot sightings. 

# Conclusion

This project has a number of sections that were interesting to work on. Web scraping the data was good practice on scraping and cleaning data from a website. The EDA showed trends which gave more insight into the reported sightings. Training a text generator was very time consuming but by comparing models and tuning hyperparameters, it was interesting to compare how the generated text did or did not provide readable sightings. GPT-2 had more natural sounding text overall, but further hyperparameter tuning for the RNN models could be fruitful in both the training and sampling phases.
...
Acknowledgements to the Galvanize data science bootcamp, which this project grew from, the bigfoot field researchers organization where the data for this project was pulled, and Brandon and Eddie for help with the EDA sections when we worked on some of this for the bootcamp.
...
* [Brandon Lindsey](https://github.com/branlindsey)
* [Eddie Ressegue](https://github.com/redwin21)